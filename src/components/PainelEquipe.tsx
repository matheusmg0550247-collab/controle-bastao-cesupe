...
export function PainelEquipe() {
  ...
  const getStatusStyle = (status: string) => {
    switch (status) {
      ...
      case 'Lanche': return 'bg-yellow-100 text-yellow-800 border-yellow-300 hover:bg-yellow-200';
  ...
  const getStatusEmoji = (status: string) => {
    switch (status) {
      case 'Atividades': return '📋'; case 'Projeto': return '🏗️'; case 'Treinamento': return '🎓';
      case 'Reunião': return '📅'; case 'Almoço': return '🍽️'; case 'Lanche': return '🍔';
      case 'Sessão': return '🎙️'; case 'Atend. Presencial': return '🤝'; default: return '';
    }
  }
  ...
  const statusOrder = ['Atividades', 'Projeto', 'Treinamento', 'Reunião', 'Almoço', 'Lanche', 'Sessão', 'Atend. Presencial', 'Indisponível'];
          ...
          <div className="bg-white w-full max-w-md rounded-2xl p-6 shadow-2xl border border-gray-200">
            ...
            <div className="flex flex-col gap-3 max-h-[60vh] overflow-y-auto pr-2">
              {agrupado[statusAberto]?.map(nome => {
                const naFila = filaEproc.includes(nome) || filaJpe.includes(nome);
                return (
                  <div key={nome} className="bg-gray-50 border border-gray-200 p-3 rounded-xl cursor-pointer hover:bg-orange-50 hover:border-orange-300 transition-colors group relative">
                    <div className="flex justify-between items-center mb-1">
                      <div className="flex items-center gap-2">
                        ...
                        {naFila && (
                          <span className="text-[10px] font-black bg-orange-100 text-orange-600 px-2 py-0.5 rounded-full border border-orange-200">🔥 No Bastão</span>
                        )}
                      </div>
                      <span className="text-xs font-black bg-gray-200 text-gray-600 px-2 py-1 rounded-md">☎ {getRamal(nome)}</span>
                    </div>
                    {statusDetalhe[nome] ? (
            ...
            <button onClick={() => setStatusAberto(null)} className="w-full mt-6 bg-gray-200 text-gray-800 font-bold py-3 rounded-xl hover:bg-gray-300">Fechar</button>
          </div>
        </div>
      )}
