...
export function PainelEquipe() {
  const {
    meuLogin, statusTexto, statusDetalhe, filaEproc, filaJpe,
    updateStatus, abrirModalAtividade
  } = useBastaoStore()
  ...
  const handleClicarNome = (nome: string, statusAtual: string) => {
    // Status presencial → abre modal de registro de atividade
    if (STATUS_PRESENCIAL.includes(statusAtual)) {
      abrirModalAtividade({ consultor: nome, statusAtual, mostrarEscolhaApos: true })
      setStatusAberto(null)
      return
    }

    // Outros status → volta à fila (comportamento original)
    ...
    <>
        ...
        {statusKeys.length === 0 ? (
          ...
          <div className="flex flex-wrap gap-3">
            {statusKeys.map(status => (
              <button key={status} onClick={() => setStatusAberto(status)}
                className={`px-4 py-2 rounded-xl border-2 font-black text-sm shadow-sm transition-all active:scale-95 ${getStatusStyle(status)}`}>
          ...
          <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white w-full max-w-md rounded-2xl p-6 shadow-2xl border border-gray-200">
              <h3 className="text-xl font-black text-gray-800 mb-4 border-b pb-2 flex justify-between items-center">
                <span>
                  {STATUS_PRESENCIAL.includes(statusAberto) ? '📋 ' : ''}
                  {statusAberto}
                </span>
              ...
              {/* Dica contextual */}
              {STATUS_PRESENCIAL.includes(statusAberto) && (
                <p className="text-xs text-violet-600 bg-violet-50 border border-violet-100 rounded-xl px-3 py-2 mb-3 font-bold">
                  💡 Clique no nome para registrar a atividade e encerrar o status
                </p>
              )}
              {!STATUS_PRESENCIAL.includes(statusAberto) && statusAberto !== 'Indisponível' && (
                <p className="text-xs text-orange-600 bg-orange-50 border border-orange-100 rounded-xl px-3 py-2 mb-3 font-bold">
                  🔥 Clique no nome para devolver ao Bastão
                </p>
              )}

              <div className="flex flex-col gap-3 max-h-[60vh] overflow-y-auto pr-2">
                {agrupado[statusAberto]?.map(nome => {
                  const presencial = STATUS_PRESENCIAL.includes(statusAberto)
                  return (
                    <div
                      key={nome}
                      onClick={() => handleClicarNome(nome, statusAberto)}
                      title={presencial ? 'Clique para registrar a atividade' : 'Clique para devolver ao Bastão'}
                      className={`bg-gray-50 border border-gray-200 p-3 rounded-xl cursor-pointer transition-colors group relative
                        ${presencial
                    ...
                    >
                      <div className="flex justify-between items-center mb-1">
                        <span className={`font-bold text-gray-800 text-lg flex items-center gap-1
                          ${presencial ? 'group-hover:text-violet-700' : 'group-hover:text-orange-700'}`}>
                          {CONSULTOR_IMAGENS[nome] && (
                            <img src={CONSULTOR_IMAGENS[nome]} alt="" className="w-5 h-5 object-contain inline-block" />
                          )}
                        ...
                        <span
                          ...
                          className="text-xs font-black bg-gray-200 text-gray-600 px-2 py-1 rounded-md cursor-default
                            transition-transform duration-300 group-hover:scale-125 origin-right z-10 relative
                      ...
                      {statusDetalhe[nome] ? (
                        <p className="text-sm text-indigo-700 font-bold bg-indigo-50 p-2 rounded-lg inline-block">
                          ↳ {statusDetalhe[nome]}
                        </p>
                      ) : (
                      ...
                      {/* Overlay hover */}
                      <div className={`absolute inset-0 text-white font-black text-sm flex items-center justify-center rounded-xl opacity-0 group-hover:opacity-100 transition-opacity backdrop-blur-sm
                        ${presencial ? 'bg-violet-600/90' : 'bg-orange-500/90'}`}>
                        {presencial ? '📋 Registrar atividade' : '🔥 Voltar à Fila'}
                      </div>
                    </div>
                  )
                })}
              ...
              <button onClick={() => setStatusAberto(null)}
                className="w-full mt-6 bg-gray-200 text-gray-800 font-bold py-3 rounded-xl hover:bg-gray-300">
                Fechar
              </button>
            </div>
          </div>
      ...
      {ramalPopover && (
        <div className="fixed inset-0 z-[999] flex items-center justify-center pointer-events-none">
          <div className="bg-white rounded-3xl shadow-2xl border-2 border-orange-200 p-8 text-center pointer-events-none">
