...
export function PainelAcoes() {
  const { meuLogin, alvoSelecionado, setAlvoSelecionado, passarBastao, toggleTelefone, toggleCafe, toggleLanche, toggleSkip, toggleFila, filaEproc, filaJpe } = useBastaoStore()
    ...
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
      ...
      <div className="mb-4 bg-orange-50/50 p-4 rounded-xl border border-orange-100">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => handleAcaoAuditada(() => toggleFila(alvoSelecionado!), 'Entrar/Sair do Bastão')} className="bg-orange-500 hover:bg-orange-600 text-white font-bold px-5 py-3 rounded-xl shadow-sm active:scale-95 transition-all whitespace-nowrap">Entrar/Sair</button>
        </div>
      ...
      {/* LINHA 2: Emojis + Passar + Pular */}
      <div className="flex gap-2 items-stretch">
        <button onClick={() => handleAcaoAuditada(() => toggleTelefone(alvoSelecionado!), 'Telefone')} className="bg-gray-100 hover:bg-gray-200 py-3 px-4 rounded-xl shadow-sm active:scale-95 transition-all text-xl" title="Telefone">📞</button>
        <button onClick={() => handleAcaoAuditada(() => toggleCafe(alvoSelecionado!), 'Café')} className="bg-gray-100 hover:bg-gray-200 py-3 px-4 rounded-xl shadow-sm active:scale-95 transition-all text-xl" title="Café">☕</button>
        <button onClick={() => handleAcaoAuditada(() => toggleLanche(alvoSelecionado!), 'Lanche')} className="bg-gray-100 hover:bg-gray-200 py-3 px-4 rounded-xl shadow-sm active:scale-95 transition-all text-xl" title="Lanche">🍔</button>
        <button onClick={handlePassarAuditado} className="flex-1 bg-blue-500 hover:bg-blue-600 text-white font-bold py-3 px-4 rounded-xl shadow-sm active:scale-95 transition-all text-sm">🔄 Passar Bastão</button>
        <button onClick={() => handleAcaoAuditada(() => toggleSkip(alvoSelecionado!), 'Pular Vez')} className="bg-gray-500 hover:bg-gray-600 text-white font-bold py-3 px-4 rounded-xl shadow-sm active:scale-95 transition-all text-sm whitespace-nowrap">⏩ Pular</button>
      </div>
    </div>
  )
}
